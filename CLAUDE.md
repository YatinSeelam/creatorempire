...
**editing is off for everybody, and announced.** `EDITING_ENABLED` in `lib/editing.ts` is `false`, so `app/(dash)/editing/layout.tsx` serves `app/(dash)/editing/soon.tsx` in place of its children and the rail's Editing row wears a `soon` chip. it used to `notFound()`, which was the app contradicting itself: a row the product draws must not lead to "this does not exist".

**there is no founder bypass, and that is the point.** the version before this let `am_i_admin` through so the launch could be rehearsed, which meant the only person who ever opened this app saw the full marketplace while every student saw a coming-soon page: the one screen nobody could check was the one everybody else got. editing is not being rehearsed here, it is not wanted, so there is nothing to rehearse and no reason for two answers. dropping it also removed an `am_i_admin` round trip from every request under `/editing`.

three doors, all shut together, because a gate on the section is worthless if something else still links into it: the layout, the rail chip, and the two entry points on the dashboard — the `get an edit` button in the toolbar and the `cuts waiting on you` attention card built in `lib/dash-server.ts`. both are behind `EDITING_ENABLED` now. `soon.tsx` is deliberately not a route file, so flipping the const deletes every path to it. `EDITOR_MARKET_ENABLED` and `EDITOR_HIRING_ENABLED` were already false and still 404 the whole `/editors` tree and `/e/<handle>`.
